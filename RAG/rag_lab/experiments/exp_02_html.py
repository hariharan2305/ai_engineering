"""
Experiment 02d — HTML Header Chunking

Variable changed vs baseline:
  - Chunking: FixedSizeChunker → HTMLChunker (LangChain HTMLHeaderTextSplitter)
  - Corpus: embedding_models_guide.html (HTML-structured document)
  - Test questions: test_questions_html.json (5 questions tied to specific sections)

Why this is different from other chunking experiments:
  - Not a direct baseline comparison (different corpus + questions)
  - Tests whether HTML structure-aware chunking retrieves better answers
    than if we had stripped tags and chunked the plain text with fixed-size splits

What to observe:
  - Chunk count: one chunk per <h1>/<h2>/<h3> section
  - Chunk metadata: each chunk carries its header path — valuable for section-level filtering
  - HTMLHeaderTextSplitter parses the DOM, so chunk text is clean (no HTML tags)
  - answer_similarity: are section-aligned chunks better matched to section-specific questions?

Run: uv run python experiments/exp_02_html.py
"""
...
cfg = RAGConfig(
    vectordb=VectorDBConfig(collection_name="rag_lab_exp02_html")
)
...
def ingest(vectordb: ChromaVectorDB, embedder: SentenceTransformerEmbedder) -> int:
    corpus_file = ROOT / "corpus" / "embedding_models_guide.html"
    console.print(f"\n[bold cyan]Loading:[/] {corpus_file.name}")

    doc = load_text_file(str(corpus_file))
    console.print(f"  1 HTML document loaded ({len(doc.text)} chars)")

    chunker = HTMLChunker()
    chunks = chunk_documents([doc], chunker)
    console.print(f"  {len(chunks)} chunks created (strategy=html_headers)")

    # Show what headers were found per chunk
    for c in chunks:
        header_path = " > ".join(
...
def display_results(samples: list[EvalSample], scores: dict[str, float]) -> None:
    ...
    score_table = Table(title="RAGAS Scores — HTML Chunking", show_header=True)
...
def save_results(samples: list[EvalSample], scores: dict[str, float]) -> Path:
    ...
    output = {
        "experiment": "exp_02_html",
        "timestamp": timestamp,
        "config": cfg.model_dump(),
        "variable_changed": "chunking_strategy: fixed → html_headers",
        "corpus": "embedding_models_guide.html",
        "scores": scores,
        "samples": [
    ...
    out_path = results_dir / f"exp_02_html_{timestamp}.json"
...
# ── MAIN ──────────────────────────────────────────────────────────────────────
def main() -> None:
    console.rule("[bold]Experiment 02d — HTML Header Chunking")
    console.print("Corpus: [yellow]embedding_models_guide.html[/]")
    console.print("Strategy: [yellow]HTMLHeaderTextSplitter[/] — splits on h1, h2, h3 tags")
    ...
    test_path = ROOT / "corpus" / "test_questions_html.json"
    ...
    console.rule("[bold green]Exp 02d complete — HTML header chunking")
