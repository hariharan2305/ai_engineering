"""
Experiment 02g — LlamaIndex TokenTextSplitter

Variable changed vs baseline:
  - Chunking: FixedSizeChunker (char-based) → LITokenSplitter (token-based)
  - chunk_size value is kept at 512 — but the UNIT changes: characters → tokens

The core lesson:
  512 characters ≠ 512 tokens.

  English prose averages ~4 characters per token. So:
    chunk_size=512 chars  ≈  128 tokens per chunk
    chunk_size=512 tokens ≈  2048 chars per chunk

  By keeping the same number (512) but switching the unit, this experiment
  shows what token-aware chunking actually produces: chunks that are ~4x
  larger in characters, more coherent, and better matched to the LLM's
  actual context window limits.

Why token-aware chunking matters in production:
  LLMs have context windows measured in tokens, not characters. Code,
  non-English text, and symbols can have very different chars-per-token
  ratios. If you chunk by characters, you cannot predict how many tokens
  each chunk occupies — meaning you could silently overflow the LLM's
  context window. TokenTextSplitter gives you precise control.

What to observe:
  - Chunk COUNT will be much lower than baseline (~4x fewer chunks)
  - Each chunk covers much more content
  - answer_similarity delta vs baseline: does more context per chunk help?
  - Compare against exp_02_recursive_chunking (512 chars) to see the unit difference

Run: uv run python experiments/exp_02_li_token_splitter.py
"""
...
from components import (
    load_directory,
    LITokenSplitter, chunk_documents,
...
cfg = RAGConfig(
    vectordb=VectorDBConfig(collection_name="rag_lab_exp02_li_token")
)
...
def ingest(vectordb: ChromaVectorDB, embedder: SentenceTransformerEmbedder) -> int:
    ...
    # chunk_size=512 TOKENS — same number as baseline but different unit.
    # Expect ~4x fewer chunks with ~4x more content per chunk.
    chunker = LITokenSplitter(
        chunk_size=cfg.chunking.chunk_size,   # 512 tokens, not 512 chars
        chunk_overlap=cfg.chunking.overlap,   # 50 tokens of overlap
    )
    chunks = chunk_documents(docs, chunker)
    console.print(
        f"  {len(chunks)} chunks created "
        f"(strategy=li_token_splitter, unit=TOKENS, size={cfg.chunking.chunk_size})"
    )
    console.print(
        f"  [dim]Note: {cfg.chunking.chunk_size} tokens ≈ "
        f"~{cfg.chunking.chunk_size * 4} chars — chunks are ~4x larger than char-based baseline[/dim]"
    )
...
def display_results(samples: list[EvalSample], scores: dict[str, float]) -> None:
    ...
    score_table = Table(title="RAGAS Scores — TokenSplitter vs Baseline", show_header=True)
    score_table.add_column("Metric", style="bold")
    score_table.add_column("Baseline (512 chars)", justify="right")
    score_table.add_column("Token (512 tokens)", justify="right")
...
def save_results(samples: list[EvalSample], scores: dict[str, float]) -> Path:
    ...
    output = {
        "experiment": "exp_02_li_token_splitter",
        "timestamp": timestamp,
        "config": cfg.model_dump(),
        "variable_changed": "chunking_unit: characters → tokens (chunk_size=512 tokens)",
        "scores": scores,
        "samples": [
            {
                "question": s.question,
    ...
    out_path = results_dir / f"exp_02_li_token_splitter_{timestamp}.json"
...
# ── MAIN ──────────────────────────────────────────────────────────────────────
def main() -> None:
    console.rule("[bold]Experiment 02g — LlamaIndex TokenTextSplitter")
    console.print("Variable changed: [yellow]chunk size unit[/] characters → tokens")
    console.print(
        f"[bold]Key insight:[/] chunk_size=512 chars (baseline) vs chunk_size=512 [bold]tokens[/] (this experiment)"
    )
    console.print(
        f"Config: chunk_size={cfg.chunking.chunk_size} tokens, "
        f"overlap={cfg.chunking.overlap} tokens, "
    ...
    console.rule("[bold green]Exp 02g complete — compare chunk count vs baseline first")
